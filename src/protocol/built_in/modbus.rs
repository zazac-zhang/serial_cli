...
impl ModbusProtocol {
    ...
    pub fn parse_response(&self, data: &[u8]) -> Result<(u8, u8, Vec<u8>)> {
        match self.mode {
            ModbusMode::Rtu => {
                if data.len() < 4 {
            ...
            ModbusMode::Ascii => {
                ...
                Err(SerialError::Protocol(ProtocolError::InvalidFrame(
                    "ASCII parsing not yet implemented".to_string(),
                )))
            }
        }
    }
